...
type ExportSection = { heading: string; body: string };
type PdfLine = {
  text: string;
  font: "regular" | "bold";
  fontSize: number;
...
function toAsciiPdfText(value: string) {
  return value
    .replace(/\t/g, "  ")
    .replace(/[^\x20-\x7E]/g, "?");
}
...
function buildPdfLines(title: string, sections: ExportSection[]) {
  ...
  const lines: PdfLine[] = wrapText(title, titleLineLength).map((line) => ({
    text: line,
    font: "bold",
  ...
  for (const section of sections) {
    lines.push(
      ...wrapText(section.heading, headingLineLength).map((line) => ({
        text: line,
        font: "bold" as const,
    ...
    lines.push(
      ...wrapText(section.body, bodyLineLength).map((line) => ({
        text: line,
...
function buildPdfPages(lines: PdfLine[]) {
  ...
  for (const line of lines) {
    ...
    if (line.text) {
      ...
      commands.push(
        `BT /${fontName} ${line.fontSize} Tf 1 0 0 1 ${PDF_MARGIN.toFixed(2)} ${currentY.toFixed(2)} Tm (${escapePdfText(line.text)}) Tj ET`
      );
    }
...
export function buildCsvContent(rows: Array<Record<string, unknown>>) {
  if (!rows.length) {
    return "";
  }
...
export function buildPdfDocument(title: string, sections: ExportSection[]) {
  ...
  objects.set(
    2,
    `<< /Type /Pages /Kids [${pageObjectNumbers.map((pageObjectNumber) => `${pageObjectNumber} 0 R`).join(" ")}] /Count ${pageContents.length} >>`
  );
  ...
  pageContents.forEach((content, index) => {
    ...
    objects.set(
      pageObjectNumber,
      `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${PDF_PAGE_WIDTH.toFixed(2)} ${PDF_PAGE_HEIGHT.toFixed(2)}] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> >> /Contents ${contentObjectNumber} 0 R >>`
    );
...
export function downloadPdfReport(filename: string, title: string, sections: ExportSection[]) {
...
export function openPrintableReport(title: string, sections: ExportSection[]) {
